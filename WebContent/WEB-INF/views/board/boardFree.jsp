...
$(document).ready(function(){
	...
	$("#btnSearch").click(function(){
		location.href="/board/free?search="+$("#search").val()+"&search2="+$("#search2").val();
	});
...
<div id="contents">
	<div class="container">
		...
		<h4 style="text-align: left;">사내게시판</h4>
		<hr>
		<table class="table table-hover table-condensed">
			...
			<c:forEach items="${F_list }" var="i">

				<tr class="text-left">
					<td style="text-align: center;">${i.b_num }</td>
					<td>${i.category }</td>
					<td><a href="/board/view?b_num=${i.b_num }">${i.b_title }</a>&nbsp;<span style="color:rgb(38, 114, 165);">
						<c:if test="${i.c_cnt ne 0}">[${i.c_cnt}]</c:if></span></td>
					<td>${i.userid }</td>
					<td>${i.username }</td>
					<td>${i.hits }</td>
					<td><fmt:formatDate value="${i.b_date }" pattern="yyyy-MM-dd HH:mm" /></td>
		...
		<c:import url="/WEB-INF/views/layout/b_Fpaging.jsp" />
